...
with st.sidebar:
    ...
    for label, ok in [("API FastAPI", api_ok), ("MLflow", mlflow_ok)]:
        icon, status, color = (
    ...
    st.markdown(
        f"<p style='margin:5px 0;font-size:13px;'>🟡 <b>Airflow</b> — "
        f"<a href='{AIRFLOW_PUBLIC}' target='_blank' "
        f"style='color:#38bdf8;'>Ouvrir ↗</a></p>",
        unsafe_allow_html=True,
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.subheader("📈 Données historiques du Titanic")
    ...
    try:
        ...
        if rows:
            ...
            counts = df_hist["prediction"].value_counts().rename(lmap)
            st.bar_chart(counts)
        else:
            st.info("📭 Aucune prévision enregistrée pour l'instant.")
    except httpx.HTTPError:
        st.info("💡 Endpoint `/predictions` non disponible (bonus TP S14bis).")
    ...
    for svc in services:
        ...
        with st.container():
            st.markdown(
                ...
                f"padding:20px;border-left:5px solid {svc['color']};"
            ...
            with sc1:
                ...
                for ep in svc["endpoints"]:
                    st.markdown(
                        ...
                        f"padding:2px 8px;font-size:12px;color:#94a3b8;"
                        f"margin:2px;display:inline-block;'>`{ep}`</span>",
                        unsafe_allow_html=True,
                    )
            with sc2:
                st.markdown(
                    f"<p style='color:{badge_color};font-size:15px;"
                    f"font-weight:bold;'>{badge}</p>",
                    unsafe_allow_html=True,
                )
                st.markdown(f"[🔗 {svc['label']} ↗]({svc['url']})")
