    ...
    return [
        ...
        ModelSpec(
            ...
            build_estimator=lambda params: XGBClassifier(
                random_state=RANDOM_STATE, eval_metric="logloss", n_jobs=-1, **params
            ),
        ),
        ModelSpec(
            name="lightgbm",
...
def log_family_to_mlflow(
    ...
    with mlflow.start_run(run_name=result.spec.name, nested=True):
        ...
        if register_as is not None and model_info.registered_model_version is not None:
...
def optimize(n_trials: int = 30, cv: int = 5, use_mlflow: bool = True) -> list[FamilyResult]:
    ...
    if use_mlflow:
        setup_experiment()
        logger.info("Suivi MLflow configure")
